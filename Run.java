...
public class Run {
	public static void main(final String[] args) throws IOException {
		...
		final Checkanswer checkanswer = new Checkanswer();
		final String nr = "^\\-n\\s+\\d+\\s+\\-r\\s+\\d+$||^\\-r\\s+\\d+\\s+\\-n\\s+\\d+$";
		final String ea = "^\\-e\\s+\\S+\\s+\\-a\\s+\\S+$";
		int n = 0;
		int r = 0;
		...
		while (scan.hasNextLine()) {
			...
			// 检查命令是否正确
			final Pattern pa = Pattern.compile(nr);
			final Matcher ma = pa.matcher(command);
			final Pattern p = Pattern.compile(ea);
			...
			// 对命令进行分割
			final String[] c = command.split("\\s+");
